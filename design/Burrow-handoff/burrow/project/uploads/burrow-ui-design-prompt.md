...
Verdant Intelligence design system, every screen, and component states. Build in the **dark (hero) theme** first;
...
UI is an internal developer tool (auth-gated, no SEO) — think "tmux for cloud workspaces" with a tiling terminal
grid. It is a Brave Bear Studios product and must use the Verdant Intelligence design system.
...
## Design system — Verdant Intelligence (non-negotiable)
...
- Gold `#f0a737` is **prestige-only**: model labels, token-savings stats, session uptime, status dots, the brand wordmark division label. Never a button, body text, or full border.
...
┌───────────────────────────────────────────────────────────────────────┐
│ TOP BAR (52px)                                                          │
│ [hex mark] Burrow/STUDIOS   [den01 chip][den02 chip]   [themes][+ New]  │
├──────────────┬────────────────────────────────────────────────────────┤
...
- Left: hexagonal brand mark — **gold** hex stroke (`#f4bb62`) on a green-500 rounded square (`rx 7`). Beside it,
  "Burrow" in Space Grotesk 500 with a gold "STUDIOS" overline (9.5px, tracked 2px) underneath.
- Center: one capacity chip per Proxmox node — status dot + `den01 · 3 running · 11 GB free`, with the **numbers in
...
3. Pinned user row (border-top): initials avatar in an accent-tinted circle, username, "brave bear studios", gear.
...
- *Form state:* fields — Name, Git repo, Branch (default `main`), Node (`den01`/`den02`). Footer: secondary
...
> Tokens: `brave-bear-stack/references/design/tokens.css`. Component contracts: tech-spec §8.
