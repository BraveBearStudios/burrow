...
- [x] **Phase 16: Land Credential Backend & Reconcile Release Train** — merge PR #3 onto a green main, reconcile release-please forward from the stale 1.2.0 to v1.4.0, reconcile the secret-at-rest docs to ADR-0015, prune the merged local branch (CRED-01; CI-provable; depends on 15) (completed 2026-07-13; squash-merge `f9b1868`, green main CI, release PR #1 → v1.4.0)
- [x] **Phase 17: Repo Security & Backlog Hygiene** — Dependabot config + automated-security-fixes, CodeQL SAST on the default branch + first-run baseline, clear the 4-item todo backlog (SEC-01/02/03, ROB-01/02; CI-provable; parallelizable) (completed 2026-07-13; dependabot+codeql landed, auto-security-fixes enabled, ROB-01/02 fixed, 194 api tests green)
- [x] **Phase 18: Credential Store Frontend & Onboarding Key** — wizard admin-secret + credentials steps, admin-gated Settings/Credentials screen, audit read endpoint + panel, `X-Burrow-Admin` client, `BURROW_SECRET_KEY` auto-generation, extended leak test (CRED-02..07; CI-provable; depends on 16) (completed 2026-07-13; ship blocker cleared — api 299 + ui 136 tests green)
- [x] **Phase 19: Create-UX Async-202** — `POST /workspaces` returns 202 + a creating row, the saga runs in a tracked background task, the UI's existing 3s poll drives state; ADR-0017 (UX-01; CI-provable; parallelizable) (completed 2026-07-13; api 302 + ui 136 tests green)
- [~] **Phase 20: Signed GHCR Release & Harden-Runner Block** — cut the clean v1.4.0 tag → green signed+attested release → cosign/attestation verify → flip harden-runner audit→block from the green run's telemetry (ACC-06; CI-provable / CD; depends on 15 + 18) (2026-07-14: SIGNED RELEASE DONE — v1.4.0 burned by the GITHUB_TOKEN trigger-gap, recovered via ADR-0019 workflow_dispatch → v1.4.1 signed+attested, run 29355954285. Operator-pending: independent cosign/attestation verify + harden-runner block-flip → carried into Phase 22 ACC-05)
- [x] **Phase 21: Multi-Agent Workers Research Spike** — research-only ADR-0018 for Cursor / Copilot CLI / Codex workers, confirm the credential seam is additive; no build (AGENT-02; research-only; parallelizable) (completed 2026-07-13; ADR-0018 authored — seam additivity CONFIRMED)
