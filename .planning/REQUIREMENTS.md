...
- [ ] **UI-07**: Operator can stop a running workspace from the UI — a Stop control, enabled only when status is `running`, that calls the WS-06 stop path and reflects the resulting `stopped` state (terminal disconnects, status indicator updates)
- [ ] **UI-08**: Operator can start a stopped workspace from the UI — a Start control, enabled only when status is `stopped`, that calls the WS-07 start path (awaits ttyd health) and reflects the resulting `running` state, re-enabling the terminal
