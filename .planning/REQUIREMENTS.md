...
- [ ] **RELX-04**: `release.yml` builds every GHCR reference from a **lowercased** owner and the SBOM (syft) step authenticates to the registry, so SBOM + cosign keyless sign + SLSA attestation all succeed and published images ship **signed and attested** (not the current unsigned partial-publish).
...
- [ ] **RELX-06**: Release tags follow semver `vX.Y.Z` owned by release-please; hand-pushed milestone tags no longer trigger `release.yml`, and the stale `1.2.0` release PR is reconciled forward to **v1.4.0**.
