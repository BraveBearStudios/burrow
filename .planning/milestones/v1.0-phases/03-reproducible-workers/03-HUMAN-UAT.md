...
> **Superseded by v1.3 Phase 14.** These pending real-infra items are rolled up into and
...
1. **Real worker boot + ttyd Claude session**: re-run `cc-worker-config/lxc/host-prime/20-create-template.sh`,
   boot a real worker, confirm it fetches bootconfig, clones config+project, copies CLAUDE.md, and
...
2. **`resolve_vmid` hostname parse**: confirm the `${host##*-}` hostname-suffix parse (BURROW_HOSTNAME
   ...
   `30-network-notes.md` / ADR-0004 static-IP to VMID mapping. A wrong VMID must 404 safely. ([ASSUMED] T-03-05.)
   - Status: ⬜ pending
