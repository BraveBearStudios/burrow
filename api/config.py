...
class Settings(BaseSettings):
    ...
    # Fernet master key for the encrypted-at-rest credential store. SecretStr so it
    # never renders in __repr__ or a validation error; read only at the SecretBox
    # boundary via .get_secret_value(). Empty default keeps the Fake/CI path (which
    # has no store) green; EnvSecretKeyProvider fails fast when the store is used
    # without a key. Self-host: generated by onboarding into the gitignored .env.
    # Hosted: supplied by a KmsSecretKeyProvider instead, so the key stays out of
    # the DB either way (the field name already carries the BURROW_ prefix, so it
    # binds BURROW_SECRET_KEY without a validation_alias).
